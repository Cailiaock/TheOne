...
void CSetSkinWnd::OnColor(EventArgs * e)
{
	...
	SDemoSkin *skin = (SDemoSkin *) GETSKIN(L"Demobk",GetScale());
	if (skin)
	{
...
void CSetSkinWnd::LoadSkinConfigFormXml()
	{
		...
		if (result)
		{
			pugi::xml_node skinInf = docLoad.child(L"TheOne_SKIN_CONFIG").child(L"skinInf");
			while (skinInf)
			{
				SKIN_CONFIG_INF inf;
...
void CSetSkinWnd::OnSetSkin(EventArgs * e)
{
	...
	if (_taccess(strSkinFile, 0) != 0){
		SMessageBox(NULL, _T("无法设置当前主题,找不到系统主题文件。复制TheOnes\\TheOne\\themes\\文件夹到soui根目录!"), _T("警告"), NULL);
		return;
	}
	if (skin)
	{
